...
def objective(params):
 ...
 #   print(B_L_lv_line_10km)
    ...
    # Calculate reward function:
    Delta_V1_start = 6.875202379664026
    Delta_V2_start = 6.571262736954471
    Delta_V3_start = 7.050390962581073
    Delta_F1_start = 0.7057263216202552
    Delta_F2_start = 0.8179507258373325
    Delta_P1_start = 697.3278129025324
    Delta_P2_start = 815.767847675356
    Delta_P3_start = 632.7274691793502
    ...
    Delta_P1 = np.mean(abs(np.array(P1) + B1_P)) / Delta_P1_start
    Delta_P2 = np.mean(abs(np.array(P2) + B2_P)) / Delta_P2_start
    Delta_P3 = np.mean(abs(np.array(P3_real) - B3_P)) / Delta_P3_start
    e = Delta_V1 + Delta_V2 + Delta_V3 + Delta_F1 + Delta_F2 + Delta_P1 + Delta_P2 + Delta_P3
    print(e)
    return e
...
J = 0.0005
J_Q = 0.00005
params = np.array([B_lines,  Pdroop, Qdroop, tau, J, J_Q])
#params = np.array([Qdroop])
print(params)
bounds = Bounds([B_lines*10, Pdroop / 10, Qdroop/10, tau/10, J/10, J_Q/10], [B_lines / 10, Pdroop * 10, Qdroop * 10, tau * 10, J * 10, J_Q*10])
#bounds = Bounds([Qdroop / 10, ], [Qdroop * 10])
#asdsdf


result = minimize(objective, params, bounds=bounds, method='nelder-mead')
